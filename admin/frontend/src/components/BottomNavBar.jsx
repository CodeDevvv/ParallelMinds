    ...
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50">
      ...
      >
        <button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full bg-neutral-900/70 backdrop-blur-md border border-neutral-700 text-neutral-400 hover:text-white shadow-2xl transition-colors"
      ...
      <Transition
        ...
        leaveTo="opacity-0 scale-90"
      >
        <div className="flex items-center gap-2 bg-neutral-900/70 backdrop-blur-md border border-neutral-700 rounded-full p-2 shadow-2xl">
          <button
            onClick={() => setIsOpen(false)}
            className="relative group flex items-center justify-center h-12 w-12 rounded-full text-neutral-400 hover:bg-neutral-800 transition-colors"
              ...
              <Menu.Items className="absolute right-0 bottom-full mb-2 w-56 origin-bottom-right rounded-md bg-neutral-900 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-neutral-700">
                <div className="p-1">
                  <div className="px-3 py-2">
                    <p className="text-sm font-medium text-white truncate">{adminData?.name || "Admin"}</p>
                    <p className="text-sm truncate text-neutral-400">{adminData?.email || "admin@example.com"}</p>
                  </div>

        ...
        >
            <div className="flex items-start justify-between p-5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-white">{user.personalInfo?.name}</h3>
            ...
            <div className="flex-grow space-y-3 border-t border-neutral-800 px-5 py-4">
                <p className="flex items-center gap-2 text-sm text-neutral-400">
                    ...
                    <span>{user.location?.city || 'Unknown Location'}</span>
                </p>
                <p className="flex items-center gap-2 text-sm text-neutral-400">
                    <UserGroupIcon className="h-4 w-4 flex-shrink-0 text-neutral-500" />
                    <span>Group ID: {user.groupId || 'Not Assigned'}</span>
                </p>
                <p className="flex items-center gap-2 text-sm text-neutral-400">
                    <CalendarDaysIcon className="h-4 w-4 flex-shrink-0 text-neutral-500" />
                    <span>Joined on {new Date(user.createdAt).toLocaleDateString('en-GB')}</span>
                </p>
            </div>

...
import { useDailyTip, useUserData } from '../utils/dataQuery'; // 
...
const UserDashboard = () => {
    ...
    const fetchEvents = async () => {
      try {
        const response = await axios.get(
          `${config.API_URL}/api/user/fetchEvents?status=upcoming&forDashboard=true`,
          {
  ...
  if (!userData) {
      return <div className="text-white p-10">Error loading profile data.</div>;
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-neutral-800 p-6 rounded-lg shadow-md">
            <div className='w-full flex justify-between'>
              ...
              {events.length > 0 && <Link to="/user/events" className='text-green-600 hover:text-white'>More&gt;&gt;</Link>}
            </div>
            
            {isEventsLoading ? (
                <div className="text-neutral-400 p-4">Loading events...</div>
            ) : events.length === 0 ? (
          ...
          <div className="bg-neutral-800 border-l-4 border-amber-500 p-6 rounded-lg shadow-md">
            ...
            <p className="text-neutral-300 mt-3">
              {dailyTip || "Stay positive and keep moving forward!"}
            </p>
          </div>
        </div>
      </div>

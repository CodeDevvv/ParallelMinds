...
const Events = () => {
    ...
    const handleJoin = async (eventId, title) => {
        try {
            const response = await axios.post(`${config.API_URL}/api/user/joinEvent`, { event_id: eventId }, { withCredentials: true })
    ...
    const handleLeave = async (event_id, title) => {
        try {
            const response = await axios.delete(`${config.API_URL}/api/user/leaveEvent?id=${event_id}`, { withCredentials: true })
            const { status, message } = response.data
            if (status) {
    ...
    const TabButton = ({ name }) => (
        <button onClick={() => setActiveTab(name)} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === name ? 'bg-neutral-700 text-white' : 'text-neutral-400 hover:bg-neutral-800 hover:text-neutral-200'}`}>
            {name}
        </button>
    );
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
                        <div className="flex items-center bg-neutral-800 p-1 rounded-lg">
                            <TabButton name="Joined" />
                            <TabButton name="Upcoming" />
                            <TabButton name="Completed" />
                        </div>
                        <button onClick={() => setFilterModalOpen(true)} className="flex items-center gap-2 text-sm text-neutral-300 bg-neutral-800 hover:bg-neutral-700 px-4 py-2 rounded-md transition-colors">
